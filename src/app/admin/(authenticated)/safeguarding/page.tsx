...
export default async function SafeguardingPage({
  ...
  const viewingResolved = tab === "resolved";
  ...
  const visible = viewingResolved ? resolvedAlerts : openAlerts;

  // Hydrate student + context
  const studentIds = Array.from(new Set(visible.map((a) => a.studentId).filter(Boolean) as string[]));
  const pulseIds = Array.from(new Set(visible.map((a) => a.pulseCheckId).filter(Boolean) as string[]));
  const surveyRespIds = Array.from(new Set(visible.map((a) => a.surveyResponseId).filter(Boolean) as string[]));

  const [students, pulses, surveyResps, resolvers] = await Promise.all([
    studentIds.length > 0
    ...
    prisma.admin.findMany({
      where: { id: { in: visible.map((a) => a.resolvedByAdminId).filter(Boolean) as string[] } },
  ...
  return (
    <div className="max-w-4xl">
      <div className="flex items-start justify-between mb-6 gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 mb-1">Safeguarding alerts</h1>
          <p className="text-gray-500">
            {openAlerts.length === 0
              ? "No active alerts."
              : `${openAlerts.length} open alert${openAlerts.length === 1 ? "" : "s"} needing review.`}
          </p>
        </div>
        <Link
          href="/admin/safeguarding/policy"
      ...
      {visible.length === 0 ? (
        <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
          <p className="text-gray-500">
            {viewingResolved ? "No resolved alerts yet." : "No open alerts. 🎉"}
          </p>
        </div>
      ) : (
